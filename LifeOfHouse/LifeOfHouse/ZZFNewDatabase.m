...
@interface ZZFNewDatabase ()

@property (nonatomic,strong)FMDatabase * database;

@end

@implementation ZZFNewDatabase

+(instancetype)shareDatabase{
    
    //  静态的指针
    static ZZFNewDatabase *single = nil;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        //数据库路径
        NSString *databasePath = [NSHomeDirectory() stringByAppendingString:@"/Documents/new.rdb"];
        ...
        //创建表
        if (![_database executeUpdate:@"create table if not exists ZZFNewList (album_type,albumId,img,title)"]) {
            NSLog(@"创建表失败");
...
-(void)setModel:(ZZFNewModel *)model{
    if (![self.database executeUpdate:@"insert into ZZFNewList (album_type,albumId,img,title) values (?,?,?,?)",model.album_type,model.albumId,model.img,model.title]) {
        NSLog(@"插入失败");
    }
}
...
-(NSArray *)searchFromDatabase{
    ...
    while ([set next]) {
        ...
        //实例化对象
        ZZFNewModel * model = [ZZFNewModel new];
